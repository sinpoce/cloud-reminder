...
- 📡 **多渠道推送** — Telegram、企业微信、飞书（支持签名）、**邮箱（Resend / SMTP）**、**Bark（iOS）**、通用 Webhook；每个渠道可**自定义消息模板**，邮件另有内置 **SINPOCE** 品牌 HTML 模板
...
| 后端 | Cloudflare Workers · [Hono](https://hono.dev) · TypeScript |
| 存储 | Cloudflare **D1**（SQLite） |
| 调度 | Workers **Cron Triggers**（每分钟） |
| 鉴权 | HS256 JWT（Web Crypto 实现，零依赖） |
